...
namespace BinanceBot.Market
{
    ...
    public class MarketMakerBot : BaseMarketBot<NaiveMarketMakerStrategy>
    {
        ...
        public MarketMakerBot(
            ...
            base(logger, config, symbol, marketStrategy)
        {
        ...
        public override async Task ValidateConnectionAsync()
        {
            ...
            if (testConnectResponse != null)
            {
                ...
                Logger.LogInformation($"Connection was established successfully. Approximate ping time: {DateTime.UtcNow.Subtract(serverTimeResponse.ServerTime).TotalMilliseconds:F0} ms");
            }
        }
        ...
        private async Task OnMarketBestPairChanged(object sender, MarketBestPairChangedEventArgs e)
        {
            ...
            if (q != null)
            {
                ...
                Logger.LogInformation($"Limit order created. Price: {newOrderRequest.Price}. Volume: {newOrderRequest.Quantity}");
            }
